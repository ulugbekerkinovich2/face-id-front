...
function CheckUserModal({ open, onClose, canBlock }: { open: boolean; onClose: () => void; canBlock: boolean }) {
  const qc = useQueryClient();
  ...
  const blockMut = useMutation({
    mutationFn: () => api.blockByName(checked),
  ...
  const unblockMut = useMutation({
    mutationFn: () => api.unblockByName(checked),
...
export default function BlockedPage() {
  ...
  return (
    <div className="p-5 lg:p-6 space-y-5">
                ...
                <tr key={u.id ?? u.name} className={`border-b border-border/15 hover:bg-muted/20 transition-colors ${
                        ...
                        <div key={row.label} className="flex items-center gap-2">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {row.devices.map((fid) => {
                              ...
                              const disabled = state === "missing" || state === "unknown" || isPending || !u.name || !canBlock;
                  ...
                  <td className="px-4 py-3">
                    {canBlock ? (
                      ...
                      <span className="text-[11px] text-muted-foreground">Ruxsat yo'q</span>
                    )}
                  </td>
                </tr>
                );
      ...
      <AddBlockModal open={showAdd && canBlock} onClose={() => setShowAdd(false)} />
      ...
      <CheckUserModal open={showCheck} onClose={() => setShowCheck(false)} canBlock={canBlock} />
    </div>
  );
}
