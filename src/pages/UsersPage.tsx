...
import {
  ...
  ShieldBan, ShieldCheck, List, FileSpreadsheet, Upload,
} from "lucide-react";
...
function BulkTextModal({ open, onClose, action }: { open: boolean; onClose: () => void; action: "block" | "unblock" }) {
  ...
  const mut = useMutation({
    ...
    onMutate: () => {
      ...
      toast.loading(
        action === "block"
          ? `${count} ta user bloklanmoqda...`
          : `${count} ta user blokdan chiqarilmoqda...`,
        { id: "users-bulk-text" },
      );
    },
    onSuccess: (d: any) => {
      toast.success(
        `${d.success}/${d.total} user ${action === "block" ? "bloklandi" : "blokdan chiqarildi"}`,
        { id: "users-bulk-text", description: `Topilmadi: ${d.not_found}` },
      );
    ...
    onError: () => toast.error("Xatolik", { id: "users-bulk-text" }),
  });
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-in" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-border/30">
          <div>
            <h3 className="text-base font-bold">
              {action === "block" ? "Ko'p userni bloklash" : "Ko'p userni blokdan chiqarish"}
            </h3>
            <p className="text-[12px] text-muted-foreground mt-0.5">Passport yoki usernamelarni yozing</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-muted"><X className="w-4 h-4" /></button>
        </div>
        <div className="p-5 space-y-3">
          ...
          <p className="text-[11px] text-muted-foreground">
            {count > 0 ? `${count} ta user kiritildi` : "Har qatorda bitta user"}
          </p>
        </div>
        <div className="flex gap-2 p-5 pt-0">
          <button onClick={onClose} className="flex-1 h-10 rounded-lg text-sm font-medium bg-muted/50 hover:bg-muted">Bekor</button>
          <button
            onClick={() => mut.mutate()}
            disabled={count === 0 || mut.isPending}
            className={`flex-1 h-10 rounded-lg text-sm font-semibold text-white disabled:opacity-50 flex items-center justify-center gap-2 ${
              action === "block" ? "bg-rose-500 hover:bg-rose-600" : "bg-emerald-500 hover:bg-emerald-600"
            }`}
          >
...
function ExcelImportModal({ open, onClose, action }: { open: boolean; onClose: () => void; action: "block" | "unblock" }) {
  ...
  const mut = useMutation({
    ...
    onMutate: () => {
      toast.loading(
        action === "block" ? "Excel bo'yicha bloklanmoqda..." : "Excel bo'yicha blokdan chiqarilmoqda...",
        { id: "users-bulk-excel" },
      );
    },
    onSuccess: (d: any) => {
    ...
    onError: () => toast.error("Excel import xatoligi", { id: "users-bulk-excel" }),
  });
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl animate-in" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-border/30">
          <div>
            <h3 className="text-base font-bold">Excel bilan ko'p user</h3>
        ...
        <div className="p-5 space-y-4">
          <input ref={fileInput} type="file" accept=".xlsx,.xls" onChange={(e) => setFile(e.target.files?.[0] || null)} className="hidden" />
          <button
          ...
          >
            ...
            <p className="text-[13px] font-semibold">{file ? file.name : "Excel fayl tanlang"}</p>
...
export default function UsersPage() {
  ...
  return (
    <div className="p-5 lg:p-6 space-y-5">
          ...
          {canBulk && (
            <>
              <button onClick={() => setShowBulkText("block")}
                ...
                <List className="w-3.5 h-3.5" /> Ko'p bloklash
              </button>
              <button onClick={() => setShowExcel("block")}
                ...
                <FileSpreadsheet className="w-3.5 h-3.5 text-emerald-600" /> Excel bloklash
              </button>
              <button onClick={() => setShowBulkText("unblock")}
                ...
                <ShieldCheck className="w-3.5 h-3.5" /> Ko'p chiqarish
              </button>
              <button onClick={() => setShowExcel("unblock")}
                className="h-9 px-3 rounded-lg text-xs font-semibold bg-white border border-border hover:bg-muted flex items-center gap-2">
                <Upload className="w-3.5 h-3.5 text-emerald-600" /> Excel chiqarish
              </button>
            </>
          )}
      ...
      <BulkTextModal open={!!showBulkText && canBulk} onClose={() => setShowBulkText(null)} action={showBulkText || "block"} />
      <ExcelImportModal open={!!showExcel && canBulk} onClose={() => setShowExcel(null)} action={showExcel || "block"} />
    </div>
  );
}
